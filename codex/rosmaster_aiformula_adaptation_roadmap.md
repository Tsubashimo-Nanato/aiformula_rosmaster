...
This roadmap is based on local mirrors only. No live ROSMASTER robot files were modified.
...
- ROSMASTER: Yahboom X3-style mecanum chassis, `Rosmaster_Lib`, serial controller on `/dev/myserial`, low-level command API `set_car_motion(vx, vy, wz)`, onboard IMU/magnetometer/voltage/LED/buzzer support, Orbbec/Astra-class camera support, optional lidar packages, Yahboom app/hotspot startup.
...
- `yahboomcar_bringup/yahboomcar_bringup/Mcnamu_driver_X3.py` as the reference for motor command, raw velocity, IMU, magnetometer, voltage, and LED/buzzer topics.
- `yahboomcar_base_node/src/base_node_X3.cpp` as the reference for integrating `vel_raw` into `odom_raw`.
- `yahboomcar_description/urdf/yahboomcar_X3.urdf` as the physical geometry reference.
...
Start by bypassing Sophia's CAN motor path for actual driving.
...
- Driving the mecanum base through a fake CAN interface would hide the real hardware semantics and make safety harder.

First driving path:

```text
/aiformula_control/game_pad/cmd_vel
  -> ROSMASTER adapter/remap
  -> Yahboom X3 driver cmd_vel
  -> Rosmaster_Lib.set_car_motion(vx, vy, wz)
...
- Add conservative velocity limits matching Rosmaster X3 documented input ranges:
  - `vx`: `[-1.0, 1.0]`
  - `vy`: `[-1.0, 1.0]`
  - `wz`: `[-5.0, 5.0]`
- Default `vy` to `0.0` for AI Formula differential-drive compatibility unless a test explicitly needs mecanum lateral motion.

### Phase 3: Description And Frames

- Build a ROSMASTER description package using the X3 URDF as reference.
